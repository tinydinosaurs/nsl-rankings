...
- **2026-05-08 — Tournament/Upload UX refactor.** Standalone `/admin/upload` page and `AddTournamentModal` removed and folded into the tournament-creation flow. Adding a tournament now navigates to `/admin/tournaments/new` (single page: metadata + optional inline file picker; date defaults to today). If a file is attached, the user is taken to a dedicated `/admin/tournaments/:id/upload` preview/confirm page; the same page is used to add results to existing tournaments from their detail view. `ResultsUploadForm` gained an `initialFile` prop that auto-triggers preview when carrying a file forward via React Router state. The "Upload" nav entry was removed. References to the old `UploadPage` and `AddTournamentModal` in earlier ROADMAP entries are left in place as historical record.
