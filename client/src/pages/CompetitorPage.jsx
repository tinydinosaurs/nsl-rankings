...
export default function CompetitorPage() {
  ...
  for (const event of EVENTS) {
    ...
    eventAverages[event] = scores.length > 0
  ...
  return (
    <div className="competitor-page">
      <button className="btn-ghost back-btn" onClick={() => navigate('/')}>← Back to Rankings</button>
        ...
        {EVENTS.map(event => {
          ...
          return (
            <div className="score-card card" key={event}>
              <div className="score-card-label">{EVENT_LABELS[event]}</div>
              <div
                className="score-card-value"
                style={hue !== null ? { color: `hsl(${hue}, 70%, 65%)` } : {}}
              >
                {fmt(val)}
              </div>
              <div className="score-card-count">
                {data.history.filter(h => h[event] !== null).length} tournaments
              </div>
            </div>
          );
        })}
      ...
      {data.history.length === 0 ? (
        <div className="card empty-state">No tournament results yet.</div>
      ) : (
          ...
          <table className="history-table">
            <thead>
              <tr>
                <th>Tournament</th>
                <th>Date</th>
                {EVENTS.map(e => <th key={e}>{EVENT_LABELS[e]}</th>)}
              </tr>
            </thead>
            <tbody>
              {[...data.history].reverse().map((row, i) => (
                <tr key={i}>
                  <td>{row.tournament_name || <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                  <td>{row.tournament_date}</td>
                  {EVENTS.map(e => (
                    <td key={e} className={row[e] === null ? 'null-score' : ''}>
                      {fmt(row[e])}
                    </td>
                  ))}
                </tr>
