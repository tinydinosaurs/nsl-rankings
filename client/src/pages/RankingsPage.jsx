...
function ScoreCell({ value }) {
  if (value === null || value === undefined) return <span className="score-null">—</span>;
    ...
    columnHelper.accessor('name', {
      ...
      cell: info => (
        <button
          ...
          onClick={() => navigate(`/competitors/${info.row.original.id}`)}
        >
          {info.getValue()}
        </button>
      ),
  ...
  return (
    <div className="rankings-page">
      <div className="page-header">
        ...
        <span className="competitor-count">{data.length} competitors</span>
      </div>

      {data.length === 0 ? (
        <div className="card empty-state">
          <p>No rankings yet. Upload a CSV or add results manually to get started.</p>
        </div>
      ) : (
