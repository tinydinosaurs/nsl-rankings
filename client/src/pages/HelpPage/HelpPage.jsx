 ...
 * rationale):
 * - Layout: PageHeader on top, then a two-column body — sticky TOC sidebar on
 *   the left, scrollable content on the right. Below 900px the grid collapses
 *   to a single column with the TOC stacked on top.
 * - Content sections are capped to a comfortable reading width (~52rem)
 *   inside the wider grid cell so long prose doesn't run hard against the
 *   far edge of a 1200px viewport.
	...
	return (
		<div className="help-page">
			...
			<div className="help-page__body">
				<aside className="help-toc card" aria-label="On this page">
				...
				<div className="help-page__sections">
					<section className="card help-section" aria-labelledby="quick-start">
						...
						<p>
							You just want to upload a tournament. Here's the whole flow in
							three steps:
						</p>
						<ol>
							<li>
						...
						<p>
							If you don't have the file ready yet, save the tournament without
							a file and add results later from the tournament's detail page.
						</p>
					</section>
					...
					<section className="card help-section" aria-labelledby="uploading">
						...
						<p>
							There are three ways to get results into a tournament: upload a
							results file while you're creating the tournament, upload a
							results file later, or add scores for one competitor at a time.
							Most tournaments use a file upload, but the per-competitor option
							is handy when you only have a few entries (e.g. a late registrant)
							or when you don't have a clean file to upload at all.
						</p>
						<h3>Upload During Tournament Creation</h3>
						<ul>
							<li>
								Go to <Link to="/admin/tournaments">Tournaments</Link> and click{' '}
								<strong>Add Tournament</strong>.
						...
						<ul>
							<li>
								Go to the <Link to="/admin/tournaments">Tournaments</Link> page
						...
						<p>
							Instead of (or in addition to) uploading a results file, you can
							add a single competitor's scores directly from the tournament
							detail page. This is the right choice when you only have a handful
							of entries to record, when a competitor registered after the file
							was uploaded, or when the original file is missing someone you
							need to add now — it's faster than re-uploading the whole file.
						</p>
						<ul>
							<li>
								Go to the <Link to="/admin/tournaments">Tournaments</Link> page
								and click on the tournament.
							</li>
							<li>
								In the Results section, click <strong>Add Competitor</strong>.
							</li>
							<li>
								Pick the competitor from the list and enter their scores for
								each active event, then save.
							</li>
						</ul>
						<p className="help-note">
							If the person you need to add isn't in the picker, they don't
							exist as a competitor yet. Add them on the{' '}
							<Link to="/admin/competitors">Competitors</Link> page first, then
							come back to the tournament to record their scores.
						</p>
					</section>
					...
					<section className="card help-section" aria-labelledby="csv-format">
						...
						<ul>
							<li>
								<strong>Name</strong> — also recognized as:{' '}
								<code>competitor</code>, <code>athlete</code>,{' '}
								<code>player</code>, <code>participant</code>,{' '}
								<code>full name</code>.
							</li>
							<li>
								<strong>Email</strong> — also recognized as: <code>e-mail</code>
								,<code>email address</code>. Email is the unique identifier for
								a competitor — see{' '}
								<a href="#competitors">Managing competitors</a>.
							</li>
							<li>
								...
								saved as a non-member and the preview shows a count of how many
								rows that affected. If the column is missing entirely, the
						...
						<p className="help-note">
							...
							placeholder like{' '}
							<code>firstname.lastname.nsl@placeholder.local</code> and warns
							you. Replace it with the real email later from the competitor's
							detail page.
						</p>
						<p className="help-note">
							Note: Membership status can be added manually later on the
							competitor's profile page.
						</p>

						<h3>Event columns</h3>
						<p>
							Include a column for each event the tournament held. Only the
							events you mark as active on the tournament are required.
						</p>
						<ul>
							<li>
							...
							<li>
								<strong>Distance</strong>: <code>distance</code>,{' '}
								<code>dist</code>, <code>dst</code>.
							</li>
							<li>
							...
							<li>
								...
								<code>wood</code>, <code>woods course</code>,{' '}
								<code>forest</code>, <code>wc</code>.
							</li>
						</ul>
						<h3>What blank cells mean</h3>
						<ul>
							<li>
								<strong>Blank in an active event column</strong> = the
								competitor participated and scored zero. Counted in their
								average.
							</li>
							<li>
								<strong>Missing event column entirely</strong> (for an active
								event) = treated as zero with a warning. If the event wasn't
								held, mark the event inactive on the tournament instead.
							</li>
							<li>
								<strong>Non-score values</strong> like <code>DNS</code>,{' '}
								<code>DQ</code>, <code>DNF</code>, <code>scratch</code>,{' '}
								<code>WD</code>, <code>n/a</code>, <code>-</code> = the
								competitor did not participate in that event. Excluded from
								their average for that event (neither helps nor hurts the
								score).
							</li>
						</ul>
					</section>
					...
					<section
						...
						aria-labelledby="fixing-mistakes"
					>
						...
						<p>
							Open the tournament from{' '}
							<Link to="/admin/tournaments">Tournaments</Link>. Each result row
							has edit and delete controls. Edit fixes the score inline; delete
							removes that competitor's result for that tournament only.
						</p>
						...
						<p>
							See{' '}
							<a href="#uploading">
								Uploading a tournament → Add Scores Individually
							</a>{' '}
							for the full steps. Short version: open the tournament, click{' '}
							<strong>Add Competitor</strong>, pick the person, enter their
							scores. If they're not in the picker, add them on the{' '}
							<Link to="/admin/competitors">Competitors</Link> page first.
						</p>

						<h3>The whole file was wrong</h3>
						<p>
							On the tournament detail page, click{' '}
							<strong>Remove All Results</strong> to clear every result for that
							tournament (the tournament itself stays). Then re-upload the
							corrected file via <strong>Upload Results</strong>. This is the
							safest path when the mistake is widespread (wrong totals, swapped
							events, etc.).
						</p>
						...
						<p>
							On the tournament detail page, click{' '}
							<strong>Delete Tournament</strong>. This removes the tournament
							and all of its results. It cannot be undone — the confirmation
							dialog will ask you to type the tournament name first.
						</p>
						<p className="help-note">
							Deleting a tournament does not remove competitors added to the
							tournament. Their scores will, however, be recalcuated to reflect
							the removal.
						</p>
					</section>
					...
					>
						...
						<p>
							On the tournament detail page, the{' '}
							<strong>Edit Tournament</strong> button covers everything about
							the tournament itself: name, date, which events were held
							(active/inactive toggles), and the total possible points for each
							event.
						</p>
						<p>
							Changing event totals re-scales the percentages on existing
							results automatically — you don't need to re-enter scores.
							Toggling an event off removes it from the average for every
							competitor in that tournament; toggling it back on restores it.
						</p>
					</section>
					...
					<section className="card help-section" aria-labelledby="competitors">
						...
						<p>
							...
							<strong>email</strong>, not name. This is why "Bob Smith" and
							"Robert Smith" can show up as the same competitor across two
							tournaments — if both files use the same email, they're the same
							person. It also means changing a competitor's name on their
							profile is safe and won't break their history.
						</p>
						...
						<p>
							...
							gets a record and a ranking — but a "Placeholder email" badge
							shows up on their profile. When you get the real email, open the
						...
						<p>
							...
							show up in admin views with a "Non-member" badge, and their
							results are still recorded — they just don't count toward the
							public rankings. Toggle membership on the competitor's profile or
							via the Add/Edit Competitor forms.
						</p>
						...
						<p>
							...
							the duplicate competitor (this cascades and removes their
							results), and re-upload the affected tournaments with the correct
							email. Better duplicate-merge tooling is on the roadmap.
						</p>
					</section>
					...
					<section className="card help-section" aria-labelledby="scoring">
						...
						<ul>
							...
							<li>
								Their <strong>total score</strong> is the four event scores
								added together and divided by four. Always divided by four, even
								if they haven't competed in all four events yet.
							</li>
							<li>
					...
					<section className="card help-section" aria-labelledby="contact">
						...
						<p>
							If you spot a bug, a confusing flow, or rankings that don't add
							up, contact the league owner or board contact who set up your
							account. Include the tournament name, the competitor name (if
							relevant), and what you expected to see vs. what you actually saw
